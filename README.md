...
import {
  ...
  WeixinTextReply,
} from 'https://github.com/manyuanrong/deno-weixin-sdk/raw/v1.0.1/mod.ts';
...
const tokenManager = new MemoryAccessTokenManager();
...
sdk.setMessageHandler((msg: WeixinBaseMessage) => {
  console.log('收到微信消息');
  // 收到的消息类型是WeixinBaseMessage，是所有消息的基础类型。msg.msgType判断为指定消息之后可以通过断言转换为特定的消息类型
  if (msg.msgType === 'text') {
